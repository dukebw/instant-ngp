...
void
to_json(json& j, const CameraKeyframe& p) {
    j = json{{"R", p.R},
             {"T", p.T},
             {"slice", p.slice},
             {"scale", p.scale},
...
void
CameraPath::save(const std::string& filepath_string) {
...
void
CameraPath::load(const std::string& filepath_string,
...
#ifdef NGP_GUI
int
CameraPath::imgui(char path_filename_buf[128],
...
bool
DebugProject(const Matrix<float, 4, 4>& proj, Vector3f p, ImVec2& o) {
    Vector4f ph;
    ph << p, 1.f;
    Vector4f pa = proj * ph;
...
void
add_debug_line(const Matrix<float, 4, 4>& proj,
               ImDrawList* list,
               Vector3f a,
               Vector3f b,
               uint32_t col,
...
void
visualize_unit_cube(const Matrix<float, 4, 4>& world2proj) {
...
void
visualize_nerf_camera(const Matrix<float, 4, 4>& world2proj,
...
bool
CameraPath::imgui_viz(Matrix<float, 4, 4>& view2proj,
    ...
    if (!m_update_cam_from_path) {
        ...
        for (int i = 0; i < m_keyframes.size(); ++i) {
            ...
            if (i) add_debug_line(world2proj, list, prevp, p, 0xccffc040);
            prevp = p;
        }
        if (!m_keyframes.empty()) {
            ...
            if (ImGuizmo::Manipulate((const float*)&world2view,
