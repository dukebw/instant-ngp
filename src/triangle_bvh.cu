...
class Gas {
   public:
    Gas(const GPUMemory<Triangle>& triangles,
...
unsigned_distance_kernel(uint32_t n_elements,
                         ...
                         bool use_existing_distances_as_upper_bounds = false);
__global__ void
raytrace_kernel(uint32_t n_elements,
...
struct DistAndIdx {
    ...
    operator<(const DistAndIdx& other) {
...
class TriangleBvhWithBranchingFactor : public TriangleBvh {
   public:
    __host__ __device__ static std::pair<int, float>
    ray_intersect(const Vector3f& ro,
                  const Vector3f& rd,
                  const TriangleBvhNode* __restrict__ bvhnodes,
                  const Triangle* __restrict__ triangles) {
        ...
        while (!query_stack.empty()) {
            ...
            if (node.left_idx < 0) {
                ...
                for (uint32_t i = 0; i < BRANCHING_FACTOR; ++i) {
                    children[i] = {
                        bvhnodes[i + first_child].bb.ray_intersect(ro, rd).x(),
                        i + first_child};
                }
    ...
    __host__ __device__ static std::pair<int, float>
    closest_triangle(const Vector3f& point,
                     const TriangleBvhNode* __restrict__ bvhnodes,
                     const Triangle* __restrict__ triangles,
    ...
    __host__ __device__ static Vector3f
    avg_normal_around_point(const Vector3f& point,
                            const TriangleBvhNode* __restrict__ bvhnodes,
                            const Triangle* __restrict__ triangles) {
        FixedIntStack query_stack;
        query_stack.push(0);
    ...
    unsigned_distance(const Vector3f& point,
                      const TriangleBvhNode* __restrict__ bvhnodes,
                      const Triangle* __restrict__ triangles,
                      float max_distance_sq = MAX_DIST_SQ) {
    ...
    __host__ __device__ static float
    signed_distance_watertight(const Vector3f& point,
                               const TriangleBvhNode* __restrict__ bvhnodes,
                               const Triangle* __restrict__ triangles,
                               float max_distance_sq = MAX_DIST_SQ) {
    ...
    __host__ __device__ static float
    signed_distance_raystab(const Vector3f& point,
                            const TriangleBvhNode* __restrict__ bvhnodes,
                            const Triangle* __restrict__ triangles,
                            float max_distance_sq = MAX_DIST_SQ,
                            default_rng_t rng = {}) {
    ...
    float
    signed_distance(EMeshSdfMode mode,
    ...
    signed_distance_gpu(uint32_t n_elements,
        ...
        if (mode == EMeshSdfMode::Watertight) {
            ...
            if (m_optix.available) {
                linear_kernel(unsigned_distance_kernel,
                              0,
                              stream,
                              n_elements,
                              gpu_positions,
                ...
                if (mode == EMeshSdfMode::Raystab) {
                    ...
                    m_optix.pathescape->invoke({gpu_positions,
                                                gpu_triangles,
                                                gpu_distances,
                                                m_optix.gas->handle()},
                                               {n_elements, 1, 1},
            ...
            {
                if (mode == EMeshSdfMode::Raystab) {
                    linear_kernel(signed_distance_raystab_kernel,
                                  0,
                                  stream,
                                  n_elements,
                                  gpu_positions,
    ...
    ray_trace_gpu(uint32_t n_elements,
                  Vector3f* gpu_positions,
                  Vector3f* gpu_directions,
                  const Triangle* gpu_triangles,
                  cudaStream_t stream) override {
#ifdef NGP_OPTIX
        if (m_optix.available) {
