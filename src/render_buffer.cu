...
void
CudaSurface2D::resize(const Vector2i& size) {
    if (size == m_size) {
        return;
    }
    ...
    CUDA_CHECK_THROW(cudaMallocArray(
        &m_array, &desc, size.x(), size.y(), cudaArraySurfaceLoadStore));
...
void
GLTexture::blit_from_cuda_mapping() {
...
void
GLTexture::load(const char* fname) {
...
void
GLTexture::load(const float* data, Vector2i new_size, int n_channels) {
...
void
GLTexture::load(const uint8_t* data, Vector2i new_size, int n_channels) {
...
GLTexture::resize(const Vector2i& new_size, int n_channels, bool is_8bit) {
    ...
    glTexImage2D(GL_TEXTURE_2D,
                 ...
                 0,
                 m_format,
                 is_8bit ? GL_UNSIGNED_BYTE : GL_FLOAT,
                 nullptr);
...
const float*
GLTexture::CUDAMapping::data_cpu() {
    CUDA_CHECK_THROW(cudaMemcpy2DFromArray(m_data_cpu.data(),
                                           m_size.x() * sizeof(float) * 4,
                                           array(),
                                           0,
                                           0,
                                           m_size.x() * sizeof(float) * 4,
...
__global__ void
accumulate_kernel(Vector2i resolution,
...
__device__ Array3f
tonemap(Array3f x, ETonemapCurve curve) {
...
__global__ void
overlay_image_kernel(Vector2i resolution,
                     float alpha,
                     Array3f exposure,
...
__global__ void
overlay_false_color_kernel(Vector2i resolution,
                           Vector2i training_resolution,
                           bool to_srgb,
                           int fov_axis,
                           cudaSurfaceObject_t surface,
                           const float* error_map,
                           Vector2i error_map_resolution,
                           const float* average,
                           float brightness,
                           bool viridis) {
    uint32_t x = threadIdx.x + blockDim.x * blockIdx.x;
...
__global__ void
tonemap_kernel(Vector2i resolution,
               float exposure,
               Array4f background_color,
               Array4f* accumulate_buffer,
               EColorSpace color_space,
...
void
CudaRenderBuffer::resize(const Vector2i& size) {
...
void
CudaRenderBuffer::clear_frame_buffer(cudaStream_t stream) {
    auto res = resolution();
    CUDA_CHECK_THROW(cudaMemsetAsync(
        frame_buffer(), 0, sizeof(Array4f) * res.x() * res.y(), stream));
}

void
CudaRenderBuffer::accumulate(cudaStream_t stream) {
    auto res = resolution();
    ...
    accumulate_kernel<<<blocks, threads, 0, stream>>>(
        res, frame_buffer(), accumulate_buffer(), (float)m_spp, m_color_space);

    ++m_spp;
}

void
CudaRenderBuffer::tonemap(float exposure,
    ...
    const dim3 blocks = {div_round_up((uint32_t)res.x(), threads.x),
                         div_round_up((uint32_t)res.y(), threads.y),
                         1};
    tonemap_kernel<<<blocks, threads, 0, stream>>>(res,
                                                   exposure,
                                                   background_color,
                                                   accumulate_buffer(),
                                                   m_color_space,
...
void
CudaRenderBuffer::overlay_image(float alpha,
    ...
    const dim3 blocks = {div_round_up((uint32_t)res.x(), threads.x),
                         div_round_up((uint32_t)res.y(), threads.y),
                         1};
    overlay_image_kernel<<<blocks, threads, 0, stream>>>(res,
                                                         alpha,
                                                         exposure,
                                                         background_color,
                                                         image,
...
void
CudaRenderBuffer::overlay_false_color(Vector2i training_resolution,
                                      bool to_srgb,
                                      int fov_axis,
                                      cudaStream_t stream,
                                      const float* error_map,
