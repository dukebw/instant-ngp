    ...
    do {                                                     \
        ...
        if (res != OPTIX_SUCCESS) {                          \
            throw std::runtime_error(                        \
                std::string("Optix call '" #x "' failed.")); \
        }                                                    \
    } while (0)
...
class Program {
   public:
    Program(const char* data, size_t size, OptixDeviceContext optix) {
        ...
        OptixPipelineCompileOptions pipeline_compile_options = {};
        {
            ...
            OPTIX_CHECK_THROW_LOG(
                optixModuleCreateFromPTX(optix,
        ...
        // Shader binding table
        {
            ...
            const size_t raygen_record_size =
                sizeof(SbtRecord<typename T::RayGenData>);
            ...
            CUDA_CHECK_THROW(cudaMalloc(reinterpret_cast<void**>(&miss_record),
                                        miss_record_size));
            ...
            CUDA_CHECK_THROW(
                cudaMalloc(reinterpret_cast<void**>(&hitgroup_record),
                           hitgroup_record_size));
            SbtRecord<typename T::HitGroupData> hg_sbt;
            OPTIX_CHECK_THROW(
                optixSbtRecordPackHeader(hitgroup_prog_group, &hg_sbt));
            CUDA_CHECK_THROW(
                cudaMemcpy(reinterpret_cast<void*>(hitgroup_record),
                           &hg_sbt,
                           hitgroup_record_size,
                           cudaMemcpyHostToDevice));
            ...
            m_sbt.missRecordStrideInBytes =
                sizeof(SbtRecord<typename T::MissData>);
    ...
    void
    invoke(const typename T::Params& params,
           const uint3& dim,
           cudaStream_t stream) {
