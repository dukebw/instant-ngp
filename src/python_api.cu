...
void
Testbed::override_sdf_training_data(py::array_t<float> points,
...
pybind11::dict
Testbed::compute_marching_cubes_mesh(Eigen::Vector3i res3d,
                                     BoundingBox aabb,
                                     float thresh) {
    if (aabb.is_empty()) {
    ...
    CUDA_CHECK_THROW(cudaMemcpy(cpuverts.request().ptr,
                                m_mesh.verts.data(),
                                m_mesh.verts.size() * 3 * sizeof(float),
                                cudaMemcpyDeviceToHost));
    CUDA_CHECK_THROW(cudaMemcpy(cpunormals.request().ptr,
                                m_mesh.vert_normals.data(),
                                m_mesh.vert_normals.size() * 3 * sizeof(float),
                                cudaMemcpyDeviceToHost));
    CUDA_CHECK_THROW(cudaMemcpy(cpucolors.request().ptr,
                                m_mesh.vert_colors.data(),
                                m_mesh.vert_colors.size() * 3 * sizeof(float),
                                cudaMemcpyDeviceToHost));
    CUDA_CHECK_THROW(cudaMemcpy(cpuindices.request().ptr,
                                m_mesh.indices.data(),
...
py::array_t<float>
Testbed::render_to_cpu(int width,
                       int height,
                       int spp,
                       bool linear,
                       float start_time,
    ...
    for (int i = 0; i < spp; ++i) {
        ...
        render_frame(sample_start_cam_matrix,
