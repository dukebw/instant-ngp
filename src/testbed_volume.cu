...
Testbed::NetworkDims
Testbed::network_dims_volume() const {
    NetworkDims dims;
    dims.n_input = 3;
    dims.n_output = 4;
    dims.n_pos = 3;
...
__device__ Array4f
proc_envmap(const Vector3f& dir,
...
__device__ Array4f
proc_envmap_render(const Vector3f& dir,
                   const Vector3f& up_dir,
                   const Vector3f& sun_dir,
                   const Array3f& skycol) {
...
walk_to_next_event(default_rng_t& rng,
...
Testbed::train_volume(size_t target_batch_size,
    ...
    linear_kernel(volume_generate_training_data_kernel,
                  0,
                  stream,
    ...
    for (size_t i = 0; i < n_steps; i += GRAPH_SIZE) {
        ...
        for (uint32_t j = 0; j < GRAPH_SIZE; ++j) {
            ...
            GPUMatrix<float> training_target_matrix(
                (float*)(m_volume.training.targets.data() + training_offset),
                n_output_dims,
                batch_size);
...
__global__ void
volume_render_kernel_gt(uint32_t n_pixels,
                        Vector2i resolution,
                        default_rng_t rng,
                        BoundingBox aabb,
...
__global__ void
volume_render_kernel_step(uint32_t n_pixels,
                          Vector2i resolution,
                          default_rng_t rng,
                          BoundingBox aabb,
                          const Vector3f* __restrict__ positions_in,
...
void
Testbed::render_volume(CudaRenderBuffer& render_buffer,
...
Testbed::load_volume() {
    ...
    m_aabb = m_render_aabb = BoundingBox{Vector3f{0.5f - xsize * scale * 0.5f,
                                         ...
                                         Vector3f{0.5f + xsize * scale * 0.5f,
                                                  0.5f + ysize * scale * 0.5f,
                                                  0.5f + zsize * scale * 0.5f}};
            ...
            for (int k = metadata.indexBBox[0][2]; k < metadata.indexBBox[1][2];
                ...
                if (d > 0.001f) {
                    ...
                    float fz = ((k + 0.5f) - m_volume.world2index_offset.z()) /
                               m_volume.world2index_scale;
                    uint32_t bitidx = tcnn::morton3D(int(fx * 128.f + 0.5f),
                                                     int(fy * 128.f + 0.5f),
                                                     int(fz * 128.f + 0.5f));
                    if (bitidx < 128 * 128 * 128)
