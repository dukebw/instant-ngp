...
Testbed::set_camera_to_training_view(int trainview) {
    ...
    m_screen_center =
        Vector2f::Constant(1.0f) - m_nerf.training.dataset.metadata[0].principal_point;
}
...
Testbed::reset_camera() {
    ...
    m_scale = 1.5f;
    m_camera << 1.0f, 0.0f, 0.0f, 0.5f, 0.0f, -1.0f, 0.0f, 0.5f, 0.0f, 0.0f, -1.0f,
        0.5f;
...
Testbed::imgui() {
    ...
    if (ImGui::CollapsingHeader(
        ...
        if (m_testbed_mode == ETestbedMode::Nerf) {
            ImGui::Text(
                "Rays per batch: %d, Batch size: %d/%d",
        ...
        if (m_testbed_mode == ETestbedMode::Nerf &&
            ...
            ImGui::Combo(
                "RGB activation", (int*)&m_nerf.rgb_activation, NerfActivationStr);
            ...
            ImGui::Checkbox("Sample focal plane ~error",
    ...
    if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (m_testbed_mode == ETestbedMode::Sdf &&
            ...
            accum_reset |= ImGui::SliderFloat(
                "Inflate (offset the zero set)", &m_sdf.zero_offset, -0.25f, 0.25f);
            accum_reset |=
                ImGui::SliderFloat("Distance scale", &m_sdf.distance_scale, 0.25f, 1.f);
        ...
        if (ImGui::TreeNode("Debug visualization")) {
            ...
            if (m_testbed_mode == ETestbedMode::Nerf) {
                if (ImGui::SliderInt("Training view",
    ...
    if (ImGui::CollapsingHeader("Snapshot")) {
        ...
        if (ImGui::BeginPopupModal(
                "Snapshot load error", NULL, ImGuiWindowFlags_AlwaysAutoResize)) {
    ...
    if (m_testbed_mode == ETestbedMode::Nerf || m_testbed_mode == ETestbedMode::Sdf) {
        if (ImGui::CollapsingHeader("Marching Cubes Mesh Output")) {
            ...
            ImGui::SliderInt(
                "Res:", &m_mesh.res, 16, 2048, "%d", ImGuiSliderFlags_Logarithmic);
            ImGui::SameLine();
            ...
            ImGui::SliderFloat(
                "MC density threshold", &m_mesh.thresh, -thresh_range, thresh_range);
    ...
    if (ImGui::CollapsingHeader("Histograms of trainable encoding parameters")) {
        ...
        ImGui::PlotHistogram(
            "Means", f, m_num_levels, 0, "means", FLT_MAX, FLT_MAX, ImVec2(0, 60.f));
        for (int i = 0; i < m_num_levels; ++i) {
            f[i] = m_level_stats[i].sigma();
        }
        ImGui::PlotHistogram(
            "Sigma", f, m_num_levels, 0, "sigma", FLT_MAX, FLT_MAX, ImVec2(0, 60.f));
        for (int i = 0; i < m_num_levels; ++i) {
            f[i] = m_level_stats[i].fraczero() * 100.f;
        }
        ImGui::PlotHistogram(
            "% zero", f, m_num_levels, 0, "% zero", FLT_MAX, FLT_MAX, ImVec2(0, 60.f));
        ImGui::Separator();
...
Testbed::visualize_nerf_cameras(const Matrix<float, 4, 4>& world2proj) {
    ...
    for (int i = 0; i < m_nerf.training.n_images_for_training; ++i) {
        ...
        visualize_nerf_camera(
            world2proj, m_nerf.training.dataset.xforms[i].start, aspect, 0x40ffff40);
        visualize_nerf_camera(
            world2proj, m_nerf.training.dataset.xforms[i].end, aspect, 0x40ffff40);
        visualize_nerf_camera(
            world2proj, m_nerf.training.transforms[i].start, aspect, 0x80ffffff);
...
Testbed::draw_visualizations(const Matrix<float, 3, 4>& camera_matrix) {
    ...
    if (m_testbed_mode != ETestbedMode::Image) {
        ...
        view2proj << xyscale, 0, (float)m_window_res.x() * screen_center.x() * zscale,
            0, 0, xyscale, (float)m_window_res.y() * screen_center.y() * zscale, 0, 0,
            0, 1, 0, 0, 0, zscale, 0;
...
Testbed::keyboard_event() {
    ...
    if (ImGui::IsKeyPressed('E')) set_exposure(m_exposure + (shift ? -0.5f : 0.5f));
...
Testbed::draw_gui() {
    ...
    if (m_single_view) {
        list->AddImageQuad((ImTextureID)(size_t)m_render_textures.front()->texture(),
                           ImVec2{0.f, 0.f},
...
Testbed::draw_contents() {
    ...
    if (m_single_view) {
        ...
        float render_time_per_fullres_frame =
            m_frame_milliseconds / (float)render_res.x() / (float)render_res.y() *
        ...
        float factor = tcnn::clamp(
            std::sqrt(min_frame_time / render_time_per_fullres_frame), 1.0f / 8, 1.0f);
        if (!m_dynamic_res) {
...
Testbed::init_window(int resw, int resh, bool hidden) {
    ...
    title += ")";
    m_glfw_window =
        glfwCreateWindow(m_window_res.x(), m_window_res.y(), title.c_str(), NULL, NULL);
...
Testbed::frame() {
    ...
    if (m_testbed_mode == ETestbedMode::Sdf && m_sdf.calculate_iou_online) {
        m_sdf.iou = calculate_iou(
            m_train ? 64 * 64 * 64 : 128 * 128 * 128, m_sdf.iou_decay, false, true);
        m_sdf.iou_decay = 0.f;
    }
...
Testbed::reset_network() {
    ...
    if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") !=
        ...
        tlog::info() << "GridEncoding: "
                     ...
                     << " F=" << n_features_per_level << " T=2^" << log2_hashmap_size
                     << " L=" << m_num_levels;
    }
    ...
    if (m_testbed_mode == ETestbedMode::Nerf) {
        ...
        // Create distortion map model
        {
            ...
            m_distortion.map =
                std::make_shared<TrainableBuffer<2, 2, float>>(m_distortion.resolution);
...
Testbed::train(uint32_t n_training_steps, uint32_t batch_size) {
...

    {
        auto start = std::chrono::steady_clock::now();
        ScopeGuard timing_guard{[&]() {
            m_training_prep_milliseconds = std::chrono::duration<float, std::milli>(
                                               std::chrono::steady_clock::now() - start)
                                               .count();
        }};
        ...
        switch (m_testbed_mode) {
            case ETestbedMode::Nerf:
                training_prep_nerf(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Sdf:
                training_prep_sdf(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Image:
                training_prep_image(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Volume:
                training_prep_volume(batch_size, n_training_steps, m_training_stream);
                break;
            default:
...

    {
        auto start = std::chrono::steady_clock::now();
        ScopeGuard timing_guard{[&]() {
            m_training_milliseconds = std::chrono::duration<float, std::milli>(
                                          std::chrono::steady_clock::now() - start)
                                          .count();
        }};
...
Vector2f
Testbed::calc_focal_length(const Vector2i& resolution, int fov_axis, float zoom) const {
        ...
        case ETestbedMode::Sdf: {
            distance_fun_t distance_fun = m_render_ground_truth
                ? (distance_fun_t)[&](uint32_t n_elements,
                                      ...
                                      cudaStream_t stream) {
                m_sdf.triangle_bvh->signed_distance_gpu(n_elements,
                                                        m_sdf.mesh_sdf_mode,
    ...
    if (m_testbed_mode == ETestbedMode::Nerf) {
        ...
        if (m_render_ground_truth) {
            ...
            render_buffer.overlay_image(
                ...
                m_nerf.training.dataset.images_data.data() +
                    m_nerf.training.view *
                        (size_t)m_nerf.training.dataset.image_resolution.prod() * 4,
        ...
        if (m_nerf.training.render_error_overlay) {
            ...
            reduce_sum(
                ...
                reduce_size,
                m_inference_stream);
            render_buffer.overlay_false_color(m_nerf.training.dataset.image_resolution,
                                              to_srgb,
                                              m_fov_axis,
                                              m_inference_stream,
                                              err_data,
