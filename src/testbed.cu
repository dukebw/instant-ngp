...
json
merge_parent_network_config(const json& child, const fs::path& child_filename) {
...
void
Testbed::load_training_data(const std::string& data_path) {
    m_data_path = data_path;

    if (!m_data_path.exists()) {
        throw std::runtime_error{std::string{"Data path '"} +
                                 m_data_path.str() + "' does not exist."};
    }
...
json
Testbed::load_network_config(const fs::path& network_config_path) {
...
void
Testbed::reload_network_from_file(const std::string& network_config_path) {
    if (!network_config_path.empty()) {
        m_network_config_path = network_config_path;
    }
...
void
Testbed::handle_file(const std::string& file) {
...
void
Testbed::set_visualized_dim(int dim) {
    m_visualized_dimension = dim;
    reset_accumulation();
}

void
Testbed::translate_camera(const Vector3f& rel) {
    m_camera.col(3) += m_camera.block<3, 3>(0, 0) * rel * m_bounding_radius;
    reset_accumulation();
}

void
Testbed::set_nerf_camera_matrix(const Matrix<float, 3, 4>& cam) {
    m_camera = m_nerf.training.dataset.nerf_matrix_to_ngp(cam);
}

Vector3f
Testbed::look_at() const {
    return view_pos() + view_dir() * m_scale;
}

void
Testbed::set_look_at(const Vector3f& pos) {
    m_camera.col(3) += pos - look_at();
}

void
Testbed::set_scale(float scale) {
...
void
Testbed::set_view_dir(const Vector3f& dir) {
...
get_filename_in_data_path_with_suffix(fs::path data_path,
    ...
    if (data_path.is_directory())
        ...
        return data_path.stem().str() + "_" + default_name +
               std::string(suffix);
}

void
Testbed::compute_and_save_marching_cubes_mesh(const char* filename,
                                              Vector3i res3d,
                                              BoundingBox aabb,
                                              float thresh,
                                              bool unwrap_it) {
...
#ifdef NGP_GUI
bool
imgui_colored_button(const char* name, float hue) {
...
Testbed::imgui() {
    ...
    if (int read =
            ImGui::Begin("Camera Path", 0, ImGuiWindowFlags_NoScrollbar)) {
        static char path_filename_buf[128] = "";
        if (path_filename_buf[0] == '\0') {
            snprintf(path_filename_buf,
                     sizeof(path_filename_buf),
                     "%s",
                     get_filename_in_data_path_with_suffix(
                         m_data_path, m_network_config_path, "_cam.json")
                         .c_str());
        }
        if (m_camera_path.imgui(path_filename_buf,
                                m_frame_milliseconds,
                                m_camera,
                                m_slice_plane_z,
                                m_scale,
                                fov(),
        ...
        if (!m_camera_path.m_keyframes.empty()) {
            ...
            if (m_camera_path.m_update_cam_from_path)
                ...
                ImGui::Image(
                    (ImTextureID)(size_t)m_pip_render_texture->texture(),
                    ImVec2(w, w * 9.f / 16.f));
        }
    }
    ImGui::End();
    ...
    size_t n_bytes =
        tcnn::total_n_bytes_allocated() + g_total_n_bytes_allocated;
    ImGui::Text("Frame: %.3f ms (%.1f FPS); Mem: %s",
                m_gui_elapsed_ms,
                1000.0f / m_gui_elapsed_ms,
    ...
    if (ImGui::CollapsingHeader(
            "Training",
            m_training_data_available ? ImGuiTreeNodeFlags_DefaultOpen : 0)) {
        ...
        if (m_testbed_mode == ETestbedMode::Nerf) {
            ...
            ImGui::SameLine();
            ImGui::Checkbox("Train extrinsics",
                            &m_nerf.training.optimize_extrinsics);
            ImGui::SameLine();
            ImGui::Checkbox("Train exposure",
                            &m_nerf.training.optimize_exposure);
            ImGui::SameLine();
            ImGui::Checkbox("Train distortion",
        ...
        ImGui::SameLine();
        ImGui::DragInt(
            "Seed", (int*)&m_seed, 1.0f, 0, std::numeric_limits<int>::max());
        if (m_train) {
            ImGui::Text(
                "%s: %dms, Training: %dms",
                m_testbed_mode == ETestbedMode::Nerf ? "Grid" : "Datagen",
                (int)m_training_prep_milliseconds,
                (int)m_training_milliseconds);
        } else {
            ImGui::Text("Training paused");
        }
        if (m_testbed_mode == ETestbedMode::Nerf) {
            ImGui::Text("Rays per batch: %d, Batch size: %d/%d",
        ...
        ImGui::Text("Steps: %d, Loss: %0.6f (%0.2f dB)",
                    m_training_step,
                    m_loss_scalar,
                    linear_to_db(m_loss_scalar));
        ImGui::PlotLines(
            "loss graph",
            m_loss_graph,
            std::min(m_loss_graph_samples, 256),
            (m_loss_graph_samples < 256) ? 0 : (m_loss_graph_samples & 255),
            0,
            FLT_MAX,
            FLT_MAX,
            ImVec2(0, 50.f));
        ...
        if (m_testbed_mode == ETestbedMode::Nerf &&
            ...
            ImGui::Checkbox("Random bg color",
                            &m_nerf.training.random_bg_color);
            ImGui::SameLine();
            ImGui::Checkbox("Snap to pixel centers",
                            &m_nerf.training.snap_to_pixel_centers);
            ImGui::SliderFloat(
                "Near distance", &m_nerf.training.near_distance, 0.0f, 1.0f);
            accum_reset |= ImGui::Checkbox("Linear colors",
                                           &m_nerf.training.linear_colors);
            ImGui::Combo("Loss", (int*)&m_nerf.training.loss_type, LossTypeStr);
            ImGui::Combo("RGB activation",
                         (int*)&m_nerf.rgb_activation,
                         NerfActivationStr);
            ImGui::Combo("Density activation",
                         (int*)&m_nerf.density_activation,
                         NerfActivationStr);
            ImGui::SliderFloat(
                "Cone angle", &m_nerf.cone_angle_constant, 0.0f, 1.0f / 128.0f);
            ...
            ImGui::SliderFloat("Density grid decay",
                               &m_nerf.training.density_grid_decay,
                               0.f,
                               1.f,
                               "%.4f");
        ...
        if (m_testbed_mode == ETestbedMode::Sdf &&
            ...
            accum_reset |= ImGui::Combo(
                "Mesh SDF mode", (int*)&m_sdf.mesh_sdf_mode, MeshSdfModeStr);

            accum_reset |=
                ImGui::SliderFloat("Surface offset scale",
        ...
        if (m_testbed_mode == ETestbedMode::Image &&
            ...
            ImGui::Combo(
                "Training coords", (int*)&m_image.random_mode, RandomModeStr);
            ImGui::TreePop();
        }
        ...
        if (m_testbed_mode == ETestbedMode::Volume &&
            ...
            accum_reset |=
                ImGui::SliderFloat("Albedo", &m_volume.albedo, 0.f, 1.f);
            accum_reset |= ImGui::SliderFloat(
                "Scattering", &m_volume.scattering, -2.f, 2.f);
            accum_reset |=
                ImGui::SliderFloat("Distance Scale",
    ...
    if (ImGui::CollapsingHeader("Rendering", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        ImGui::Text("%dx%d at %d spp",
                    render_tex.resolution().x(),
                    render_tex.resolution().y(),
                    render_tex.spp());
        ImGui::SliderInt(
            "Max spp",
            &m_max_spp,
        ...
        if (!m_dynamic_res) {
            ImGui::SliderInt(
                "Fixed resolution factor", &m_fixed_res_factor, 8, 64);
        }

        if (m_testbed_mode == ETestbedMode::Nerf &&
            m_nerf_network->n_extra_dims() == 3) {
            ...
            if (ImGui::TreeNodeEx("Light Dir (Polar)",
                ...
                bool spin = ImGui::SliderFloat(
                    "Light Dir Theta", &theta, -PI() / 2.0f, PI() / 2.0f);
                ...
                if (spin) {
                    ...
                    m_nerf.light_dir = {
                        sin_phi * cos_theta, sinf(theta), cos_phi * cos_theta};
                    accum_reset = true;
                }
                ImGui::TreePop();
            }
            if (ImGui::TreeNode("Light Dir (Cartesian)")) {
                accum_reset |=
                    ImGui::SliderFloat("Light Dir X",
                                       ((float*)(&m_nerf.light_dir)) + 0,
                                       -1.0f,
                                       1.0f);
                accum_reset |=
                    ImGui::SliderFloat("Light Dir Y",
                                       ((float*)(&m_nerf.light_dir)) + 1,
                                       -1.0f,
                                       1.0f);
                accum_reset |=
                    ImGui::SliderFloat("Light Dir Z",
                                       ((float*)(&m_nerf.light_dir)) + 2,
                                       -1.0f,
                                       1.0f);
                ImGui::TreePop();
            }
        ...
        accum_reset |= ImGui::Combo(
            "Tonemap curve", (int*)&m_tonemap_curve, TonemapCurveStr);
        ...
        if (ImGui::SliderFloat("Crop size",
            ...
            if (old_render_diam > 0.f && render_diam > 0.f) {
                const Vector3f center =
                    (m_render_aabb.max + m_render_aabb.min) * 0.5f;
                float scale = render_diam / old_render_diam;
                m_render_aabb.max =
                    ((m_render_aabb.max - center) * scale + center)
                        .cwiseMin(m_aabb.max);
                m_render_aabb.min =
                    ((m_render_aabb.min - center) * scale + center)
                        .cwiseMax(m_aabb.min);
            }
        }

        if (ImGui::TreeNode("Crop aabb")) {
            accum_reset |= ImGui::SliderFloat("Min x",
                                              ((float*)&m_render_aabb.min) + 0,
                                              m_aabb.min.x(),
                                              m_render_aabb.max.x(),
                                              "%.3f");
            accum_reset |= ImGui::SliderFloat("Min y",
                                              ((float*)&m_render_aabb.min) + 1,
                                              m_aabb.min.y(),
                                              m_render_aabb.max.y(),
                                              "%.3f");
            accum_reset |= ImGui::SliderFloat("Min z",
                                              ((float*)&m_render_aabb.min) + 2,
                                              m_aabb.min.z(),
                                              m_render_aabb.max.z(),
                                              "%.3f");
            ImGui::Separator();
            accum_reset |= ImGui::SliderFloat("Max x",
                                              ((float*)&m_render_aabb.max) + 0,
                                              m_render_aabb.min.x(),
                                              m_aabb.max.x(),
                                              "%.3f");
            accum_reset |= ImGui::SliderFloat("Max y",
                                              ((float*)&m_render_aabb.max) + 1,
                                              m_render_aabb.min.y(),
                                              m_aabb.max.y(),
                                              "%.3f");
            accum_reset |= ImGui::SliderFloat("Max z",
                                              ((float*)&m_render_aabb.max) + 2,
                                              m_render_aabb.min.z(),
                                              m_aabb.max.z(),
                                              "%.3f");
            ImGui::TreePop();
        ...
        if (m_testbed_mode == ETestbedMode::Nerf &&
            ...
            accum_reset |= ImGui::Checkbox(
                "Apply lens distortion", &m_nerf.render_with_camera_distortion);
            if (m_nerf.render_with_camera_distortion) {
                accum_reset |=
                    ImGui::Combo("Distortion mode",
                                 (int*)&m_nerf.render_distortion.mode,
                                 "None\0Iterative\0F-Theta\0");
                if (m_nerf.render_distortion.mode ==
                    ...
                    accum_reset |=
                        ImGui::InputFloat("width",
                                          &m_nerf.render_distortion.params[5],
                                          0.f,
                                          0.f,
                                          "%.0f");
                    accum_reset |=
                        ImGui::InputFloat("height",
                                          &m_nerf.render_distortion.params[6],
                                          0.f,
                                          0.f,
                                          "%.0f");
                    accum_reset |=
                        ImGui::InputFloat("f_theta p0",
                                          &m_nerf.render_distortion.params[0],
                                          0.f,
                                          0.f,
                                          "%.5f");
                    accum_reset |=
                        ImGui::InputFloat("f_theta p1",
                                          &m_nerf.render_distortion.params[1],
                                          0.f,
                                          0.f,
                                          "%.5f");
                    accum_reset |=
                        ImGui::InputFloat("f_theta p2",
                                          &m_nerf.render_distortion.params[2],
                                          0.f,
                                          0.f,
                                          "%.5f");
                    accum_reset |=
                        ImGui::InputFloat("f_theta p3",
                                          &m_nerf.render_distortion.params[3],
                                          0.f,
                                          0.f,
                                          "%.5f");
                    accum_reset |=
                        ImGui::InputFloat("f_theta p4",
        ...
        if (m_testbed_mode == ETestbedMode::Sdf &&
            ...
            accum_reset |= ImGui::SliderFloat("Normals epsilon",
            ...
            accum_reset |= ImGui::SliderFloat("Maximum distance",
                                              ...
                                              "%.6g",
                                              ImGuiSliderFlags_Logarithmic);
            accum_reset |= ImGui::SliderFloat("Shadow sharpness",
            ...
            accum_reset |= ImGui::SliderFloat("Inflate (offset the zero set)",
                                              ...
                                              0.25f);
            accum_reset |= ImGui::SliderFloat(
                "Distance scale", &m_sdf.distance_scale, 0.25f, 1.f);
        ...
        if (ImGui::TreeNode("Debug visualization")) {
            ...
            if (m_testbed_mode == ETestbedMode::Nerf) {
                ...
                accum_reset |= ImGui::SliderInt(
                    "Show acceleration", &m_nerf.show_accel, -1, 7);
            }

            if (!m_single_view) {
                ImGui::BeginDisabled();
            }
            if (ImGui::SliderInt("Visualized dimension",
            ...
            if (m_testbed_mode == ETestbedMode::Nerf) {
                ...
                if (m_nerf.training.optimize_exposure) {
                    ...
                    for (uint32_t i = 0; i < m_nerf.training.dataset.n_images;
                         ++i) {
                        exposures[i] =
                            m_nerf.training.cam_exposure[i].variable().x();
                    }
    ...
    if (ImGui::CollapsingHeader("Camera", ImGuiTreeNodeFlags_DefaultOpen)) {
        ...
        if (ImGui::TreeNode("Advanced camera settings")) {
            accum_reset |= ImGui::SliderFloat2(
                "Screen center", &m_screen_center.x(), 0.f, 1.f);
            accum_reset |= ImGui::SliderFloat("Slice / Focus depth",
                                              &m_slice_plane_z,
                                              -m_bounding_radius,
                                              m_bounding_radius);
            char buf[2048];
    ...
    if (ImGui::CollapsingHeader("Snapshot")) {
        ...
        if (snapshot_filename_buf[0] == '\0') {
            snprintf(snapshot_filename_buf,
        ...
        if (ImGui::BeginPopupModal("Snapshot load error",
                                   NULL,
        ...
        ImGui::Checkbox("w/ Optimizer State",
                        &m_include_optimizer_state_in_snapshot);
        ImGui::InputText(
            "File", snapshot_filename_buf, sizeof(snapshot_filename_buf));
    }
    ...
    if (m_testbed_mode == ETestbedMode::Nerf ||
        ...
        if (ImGui::CollapsingHeader("Marching Cubes Mesh Output")) {
            ...
            if (imgui_colored_button("Save density PNG", -0.4f)) {
                ...
                snprintf(fname,
                         sizeof(fname),
                         "density_slices_%dx%dx%d.png",
                         res3d.x(),
                         res3d.y(),
                         res3d.z());
                GPUMemory<float> density = get_density_on_grid(res3d, aabb);
                save_density_grid_to_png(density,
                                         (m_data_path / fname).str().c_str(),
                                         res3d,
                                         m_mesh.thresh,
                                         flip_y_and_z_axes);
            }
            ...
            if (m_testbed_mode == ETestbedMode::Nerf) {
                ...
                if (imgui_colored_button("Save RGBA PNG sequence", 0.2f)) {
                    ...
                    GPUMemory<Array4f> rgba =
                        get_rgba_on_grid(res3d, effective_view_dir);
                    auto dir = m_data_path / "rgba_slices";
                    if (!dir.exists()) {
                        fs::create_directory(dir);
                    }
                    save_rgba_grid_to_png_sequence(
                        rgba, dir.str().c_str(), res3d, flip_y_and_z_axes);
                }
            }
            ...
            float thresh_range =
                (m_testbed_mode == ETestbedMode::Sdf) ? 0.5f : 10.f;
            ImGui::SliderFloat("MC density threshold",
                               &m_mesh.thresh,
                               -thresh_range,
                               thresh_range);
            ImGui::Combo("Mesh render mode",
            ...
            if (uint32_t tricount = m_mesh.indices.size() / 3) {
                ImGui::InputText(
                    "##OBJFile", obj_filename_buf, sizeof(obj_filename_buf));
                ...
                ImGui::SliderFloat(
                    "Laplacian smoothing", &m_mesh.smooth_amount, 0.f, 2048.f);
                ImGui::SliderFloat(
                    "Density push", &m_mesh.density_amount, 0.f, 128.f);
                ImGui::SliderFloat(
                    "Inflate", &m_mesh.inflate_amount, 0.f, 128.f);
            }
        }
    }
    ...
    if (m_testbed_mode == ETestbedMode::Sdf) {
        if (ImGui::CollapsingHeader("BRDF parameters")) {
            ...
            accum_reset |= ImGui::SliderFloat(
                "Roughness", &m_sdf.brdf.roughness, 0.f, 1.f);
            accum_reset |=
            ...
            ImGui::Separator();
            accum_reset |= ImGui::SliderFloat(
                "Subsurface", &m_sdf.brdf.subsurface, 0.f, 1.f);
            accum_reset |=
                ImGui::SliderFloat("Sheen", &m_sdf.brdf.sheen, 0.f, 1.f);
            accum_reset |= ImGui::SliderFloat(
                "Clearcoat", &m_sdf.brdf.clearcoat, 0.f, 1.f);
            accum_reset |= ImGui::SliderFloat(
                "Clearcoat gloss", &m_sdf.brdf.clearcoat_gloss, 0.f, 1.f);
        }
    ...
    if (ImGui::CollapsingHeader(
            "Histograms of trainable encoding parameters")) {
        ...
        ImGui::Separator();
        ImGui::SliderInt(
            "Show details for level", &m_histo_level, 0, m_num_levels - 1);
        if (m_histo_level < m_num_levels) {
            ...
            ImGui::PlotHistogram("Values histogram",
                                 m_histo,
                                 257,
                                 0,
                                 "",
                                 ...
                                 ImVec2(0, 120.f));
            ImGui::SliderFloat(
                "Histogram horizontal scale", &m_histo_scale, 0.01f, 2.f);
            ...
            ImGui::Text(
                "Num Zero: %d (%0.1f%%)", s.numzero, s.fraczero() * 100.f);
        }
    }
...
void
Testbed::visualize_nerf_cameras(const Matrix<float, 4, 4>& world2proj) {
    ImDrawList* list = ImGui::GetForegroundDrawList();
    for (int i = 0; i < m_nerf.training.n_images_for_training; ++i) {
        float aspect = float(m_nerf.training.dataset.image_resolution.x()) /
                       float(m_nerf.training.dataset.image_resolution.y());
        visualize_nerf_camera(world2proj,
                              m_nerf.training.dataset.xforms[i].start,
                              aspect,
                              0x40ffff40);
        visualize_nerf_camera(world2proj,
                              m_nerf.training.dataset.xforms[i].end,
                              aspect,
                              0x40ffff40);
        visualize_nerf_camera(world2proj,
                              m_nerf.training.transforms[i].start,
                              aspect,
                              0x80ffffff);
...
void
Testbed::draw_visualizations(const Matrix<float, 3, 4>& camera_matrix) {
    ...
    if (m_testbed_mode != ETestbedMode::Image) {
        ...
        view2proj << xyscale, 0,
            ...
            (float)m_window_res.y() * screen_center.y() * zscale, 0, 0, 0, 1, 0,
            0, 0, zscale, 0;
        ...
        if (m_camera_path.imgui_viz(
                view2proj, world2proj, world2view, focal, aspect)) {
            m_pip_render_surface->reset_accumulation();
        }
    }
}
...
Testbed::keyboard_event() {
    ...
    if (ImGui::IsKeyPressed('.')) {
        if (m_single_view) {
            if (m_visualized_dimension ==
                    m_network->width(m_visualized_layer) - 1 &&
                m_visualized_layer < m_network->num_forward_activations() - 1) {
                set_visualized_layer(std::max(
                    0,
                    std::min((int)m_network->num_forward_activations() - 1,
                             m_visualized_layer + 1)));
                set_visualized_dim(0);
            } else {
                set_visualized_dim(std::max(
                    -1,
                    std::min((int)m_network->width(m_visualized_layer) - 1,
    ...
    if (ImGui::IsKeyPressed('T')) set_train(!m_train);
...
void
Testbed::mouse_wheel(Vector2f m, float delta) {
    if (delta == 0) {
        return;
    }
...
void
Testbed::mouse_drag(const Vector2f& rel, int button) {
    ...
    if (is_left_held) {
        if (shift) {
            ...
            Matrix3f rot =
                (AngleAxisf(
                     static_cast<float>(-rel.x() * 2 * PI() * rot_sensitivity),
                     up) *  // Scroll sideways around up vector
                 AngleAxisf(
                     static_cast<float>(-rel.y() * 2 * PI() * rot_sensitivity),
                     side))
    ...
    if (is_right_held) {
        Matrix3f rot =
            (AngleAxisf(static_cast<float>(-rel.x() * 2 * PI()),
                        up) *  // Scroll sideways around up vector
...
Testbed::handle_user_input() {
    ...
    if (!ImGui::IsAnyItemActive() && !ImGuizmo::IsUsing() &&
        ...
        relm = {relm.x / (float)m_window_res.y(),
                relm.y / (float)m_window_res.y()};
    }
...
Testbed::draw_gui() {
    ...
    if (m_single_view) {
        list->AddImageQuad(
            (ImTextureID)(size_t)m_render_textures.front()->texture(),
            ImVec2{0.f, 0.f},
            ImVec2{(float)display_w, 0.f},
            ImVec2{(float)display_w, (float)display_h},
            ImVec2{0.f, (float)display_h},
            ImVec2(0, 0),
            ImVec2(1, 0),
            ImVec2(1, 1),
            ImVec2(0, 1));
...
Testbed::draw_contents() {
    ...
    ScopeGuard timing_guard{[&]() {
        m_frame_milliseconds = std::chrono::duration<float, std::milli>(
    ...
    if (m_single_view) {
        ...
        float render_time_per_fullres_frame =
            m_frame_milliseconds / (float)render_res.x() /
            (float)render_res.y() * (float)m_window_res.x() *
            (float)m_window_res.y();
        float min_frame_time = m_train ? 50 : 100;

        // Make sure we don't starve training with slow rendering
        float factor = tcnn::clamp(
            std::sqrt(min_frame_time / render_time_per_fullres_frame),
            1.0f / 8,
            1.0f);
        if (!m_dynamic_res) {
            factor =
                tcnn::clamp(8.f / (float)m_fixed_res_factor, 1.f / 8.f, 1.0f);
        }
        ...
        if (m_max_spp <= 0 || m_render_surfaces.front().spp() < m_max_spp)
            render_frame(m_smoothed_camera,
                         m_smoothed_camera,
                         Eigen::Vector4f::Zero(),
                         m_render_surfaces.front());
        ...
        if (m_picture_in_picture_res > 0) {
            Vector2i res(m_picture_in_picture_res,
                         m_picture_in_picture_res * 9 / 16);
...
void
Testbed::init_window(int resw, int resh, bool hidden) {
#ifndef NGP_GUI
    throw std::runtime_error{
    ...
    m_glfw_window = glfwCreateWindow(
        m_window_res.x(), m_window_res.y(), title.c_str(), NULL, NULL);
    ...
    io.ConfigInputTrickleEventQueue =
        false;  // new ImGui event handling seems to make camera controls laggy
                // if this is true.
...
fs::path
Testbed::training_data_path() const {
...
void
Testbed::apply_camera_smoothing(float elapsed_ms) {
...
CameraKeyframe
Testbed::copy_camera_to_keyframe() const {
    return CameraKeyframe(m_camera, m_slice_plane_z, m_scale, fov(), m_dof);
}

void
Testbed::set_camera_from_keyframe(const CameraKeyframe& k) {
    m_camera = k.m();
...
size_t
Testbed::n_encoding_params() {
    return m_network->n_params() - first_encoder_param();
}

size_t
Testbed::first_encoder_param() {
...
uint32_t
Testbed::network_width(uint32_t layer) const {
    return m_network->width(layer);
}

uint32_t
Testbed::network_num_forward_activations() const {
    return m_network->num_forward_activations();
}

void
Testbed::set_max_level(float maxlevel) {
    if (!m_network) return;
    auto hg_enc =
...
void
Testbed::set_min_level(float minlevel) {
    if (!m_network) return;
    auto hg_enc =
        dynamic_cast<GridEncoding<network_precision_t>*>(m_encoding.get());
    if (hg_enc) {
...
void
Testbed::set_visualized_layer(int layer) {
...
ELossType
Testbed::string_to_loss_type(const std::string& str) {
...
Testbed::reset_network() {
    ...
    if (to_lower(encoding_config.value("otype", "OneBlob")).find("grid") !=
        ...
        float desired_resolution =
            2048.0f;  // Desired resolution of the finest hashgrid level over
                      // the unit cube
        if (m_testbed_mode == ETestbedMode::Image) {
        ...
        tlog::info() << "GridEncoding: "
                     ...
                     << " b=" << m_per_level_scale
                     << " F=" << n_features_per_level << " T=2^"
                     << log2_hashmap_size << " L=" << m_num_levels;
    }
    ...
    if (m_testbed_mode == ETestbedMode::Nerf) {
        ...
        m_nerf.training.cam_focal_length_offset =
            AdamOptimizer<Vector2f>(1e-5f);
        ...
        uint32_t n_extra_dims =
            m_nerf.training.dataset.has_light_dirs ? 3u : 0u;
        ...
        if (encoding_config.contains("otype") &&
            ...
            if (!m_sdf.triangle_octree ||
                ...
                m_sdf.triangle_octree->build(*m_sdf.triangle_bvh,
                                             m_sdf.triangles_cpu,
    ...
    // Create envmap model
    {
        ...
        m_envmap.optimizer.reset(
            create_optimizer<float>(envmap_optimizer_config));
        m_envmap.trainer = std::make_shared<Trainer<float, float, float>>(
            m_envmap.envmap,
            m_envmap.optimizer,
            std::shared_ptr<Loss<float>>{
                create_loss<float>(envmap_loss_config)},
            m_seed);
...
Testbed::Testbed(ETestbedMode mode) : m_testbed_mode(mode) {
    ...
    if (compute_capability < MIN_GPU_ARCH) {
        tlog::warning() << "Insufficient compute capability "
...
void
Testbed::train(uint32_t n_training_steps, uint32_t batch_size) {
    if (!m_training_data_available) {
        m_train = false;
        return;
    }
...

    {
        ...
        switch (m_testbed_mode) {
            case ETestbedMode::Nerf:
                training_prep_nerf(
                    batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Sdf:
                training_prep_sdf(
                    batch_size, n_training_steps, m_training_stream);
...

    {
        ...
        switch (m_testbed_mode) {
            case ETestbedMode::Nerf:
                train_nerf(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Sdf:
                train_sdf(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Image:
                train_image(batch_size, n_training_steps, m_training_stream);
                break;
            case ETestbedMode::Volume:
                train_volume(batch_size, n_training_steps, m_training_stream);
                break;
            default:
                throw std::runtime_error{"Invalid training mode."};
        }
...
Vector2f
Testbed::calc_focal_length(const Vector2i& resolution,
                           int fov_axis,
                           float zoom) const {
...
void
Testbed::render_frame(const Matrix<float, 3, 4>& camera_matrix0,
    ...
    Vector2f focal_length =
        calc_focal_length(render_buffer.resolution(), m_fov_axis, m_zoom);
    Vector2f screen_center = render_screen_center();

    switch (m_testbed_mode) {
        case ETestbedMode::Nerf:
            if (!m_render_ground_truth) {
                render_nerf(render_buffer,
                            max_res,
                            focal_length,
                            camera_matrix0,
                            camera_matrix1,
        ...
        case ETestbedMode::Sdf: {
            distance_fun_t distance_fun = m_render_ground_truth
                ? (distance_fun_t)[&](uint32_t n_elements,
                                      const GPUMemory<Vector3f>& positions,
                ...
                n_elements =
                    next_multiple(n_elements, tcnn::batch_size_granularity);

                GPUMatrix<float> positions_matrix(
                    (float*)positions.data(), 3, n_elements);
                GPUMatrix<float, RM> distances_matrix(
                    distances.data(), 1, n_elements);
                m_network->inference(
                    stream, positions_matrix, distances_matrix);
            };
            ...
            normals_fun_t normals_fun =
                ...
                GPUMatrix<float> positions_matrix(
                    (float*)positions.data(), 3, n_elements);
                GPUMatrix<float> normals_matrix(
                    (float*)normals.data(), 3, n_elements);
                m_network->input_gradient(
                    stream, 0, positions_matrix, normals_matrix);
            };

            render_sdf(distance_fun,
                       normals_fun,
                       render_buffer,
                       max_res,
                       focal_length,
        ...
        if (m_render_ground_truth) {
            ...
            render_buffer.overlay_image(
                ...
                Array3f::Constant(m_exposure) +
                    m_nerf.training.cam_exposure[m_nerf.training.view]
                        .variable(),
                m_background_color,
                to_srgb ? EColorSpace::SRGB : EColorSpace::Linear,
                m_nerf.training.dataset.images_data.data() +
                    m_nerf.training.view *
                        (size_t)
                            m_nerf.training.dataset.image_resolution.prod() *
                        4,
...
void
Testbed::determine_autofocus_target_from_pixel(const Vector2i& focus_pixel) {
    float median_depth;
    ...
    // Actual depth rendering for autofocus
    {
        ...
        m_zoom = 128.0f;
        m_screen_center =
            focus_pixel.cast<float>().cwiseQuotient(m_window_res.cast<float>());
        m_dof = 0.0f;
        m_autofocus = false;
        ...
        render_frame(m_smoothed_camera,
                     ...
                     false);
        CUDA_CHECK_THROW(cudaDeviceSynchronize());
        std::vector<float> depth_cpu(depth_render_res.y() *
                                     depth_render_res.x() * 4);
        CUDA_CHECK_THROW(cudaMemcpy2DFromArray(
            depth_cpu.data(),
            depth_render_res.x() * sizeof(float) * 4,
        ...
        if (usable_depth.empty()) {
            ...
            std::nth_element(
                std::begin(usable_depth), m, std::end(usable_depth));
            median_depth = *m;
...
Testbed::autofocus() {
    float new_slice_plane_z =
        std::max(view_dir().dot(m_autofocus_target - view_pos()), 0.1f) -
        m_scale;
...
Testbed::gather_histograms() {
    ...
    if (n_encoding_params > 0 && m_trainer->params()) {
        ...
        CUDA_CHECK_THROW(cudaMemcpyAsync(grid.data(),
                                         ...
                                         n_encoding_params * sizeof(float),
                                         cudaMemcpyDeviceToHost,
                                         m_training_stream));
        CUDA_CHECK_THROW(cudaStreamSynchronize(m_training_stream));
        ...
        if (m_histo_level < m_num_levels) {
            ...
            float scale =
                128.f / (m_histo_scale);  // fixed scale for now to make it more
...
void
Testbed::save_snapshot(const std::string& filepath_string,
                       bool include_optimizer_state) {
    fs::path filepath = filepath_string;
    m_network_config["snapshot"] =
        m_trainer->serialize(include_optimizer_state);

    if (m_testbed_mode == ETestbedMode::Nerf) {
        m_network_config["snapshot"]["density_grid_size"] = NERF_GRIDSIZE();
        m_network_config["snapshot"]["density_grid_binary"] =
            m_nerf.density_grid;
    }
    ...
    if (m_testbed_mode == ETestbedMode::Nerf) {
        ...
        m_network_config["snapshot"]["nerf"]["rgb"]
                        ["measured_batch_size_before_compaction"] =
                            m_nerf.training.counters_rgb
                                .measured_batch_size_before_compaction;
        m_network_config["snapshot"]["nerf"]["dataset"] =
...
void
Testbed::load_snapshot(const std::string& filepath_string) {
    ...
    if (m_testbed_mode == ETestbedMode::Nerf) {
        ...
        if (m_network_config["snapshot"]["density_grid_size"] !=
            NERF_GRIDSIZE()) {
...
void
Testbed::load_camera_path(const std::string& filepath_string) {
