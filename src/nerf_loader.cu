...
__global__ void
from_fullp(const uint64_t num_elements,
...
__global__ void
sharpen(const uint64_t num_pixels,
        const uint32_t w,
        const __half* __restrict__ pix,
        __half* __restrict__ destpix,
        float center_w,
        float inv_totalw) {
    const uint64_t i = threadIdx.x + blockIdx.x * blockDim.x;
    if (i >= num_pixels) return;
    float rgba[4] = {__half2float(pix[i * 4 + 0]) * center_w,
    ...
    for (int j = 0; j < 4; ++j) rgba[j] -= __half2float(pix[i2++]);
    for (int j = 0; j < 4; ++j)
        destpix[i * 4 + j] = (__half)max(0.f, rgba[j] * inv_totalw);
}

__device__ inline float
luma(__half c[4]) {
    return float(c[0]) * 0.2126f + float(c[1]) * 0.7152f +
           float(c[2]) * 0.0722f;
}

__global__ void
compute_sharpness(Eigen::Vector2i sharpness_resolution,
                  Eigen::Vector2i image_resolution,
                  uint32_t n_images,
                  const __half* __restrict__ images_data,
    ...
    if (x >= sharpness_resolution.x() || y >= sharpness_resolution.y() ||
        i >= n_images)
        return;
    const size_t sharp_size =
        sharpness_resolution.x() * sharpness_resolution.y();
    ...
    for (int yy = y1; yy < y2; ++yy) {
        ...
        for (int xx = x1; xx < x2; ++xx, ++idx) {
            ...
            *(uint64_t*)&n[0] =
                ((const uint64_t*)images_data)[idx - image_resolution.x()];
...
load_nerf(const std::vector<filesystem::path>& jsonpaths,
    ...
    std::transform(jsonpaths.begin(),
                   jsonpaths.end(),
                   std::back_inserter(jsons),
                   [](const auto& path) {
                       return nlohmann::json::parse(
                           std::ifstream{path.str()}, nullptr, true, true);
                   });
    ...
    for (size_t i = 0; i < jsons.size(); ++i) {
        ...
        float sharpness_discard_threshold = json.value(
            "sharpness_discard_threshold", 0.0f);  // Keep all by default
        ...
        if (frames[0].contains("sharpness")) {
            ...
            for (int i = 0; i < (int)frames_copy.size(); ++i) {
                ...
                int mean_end = std::min(i + neighborhood_size,
                                        (int)frames_copy.size() - 1);
                ...
                if ((basepath /
                     fs::path(std::string(frames_copy[i]["file_path"])))
                        .exists() &&
                    frames_copy[i]["sharpness"] >
                        sharpness_discard_threshold * mean_sharpness) {
                    frames.emplace_back(frames_copy[i]);
                } else {
                    // tlog::info() << "discarding frame " <<
                    // frames_copy[i]["file_path"]; fs::remove(basepath /
                    // fs::path(std::string(frames_copy[i]["file_path"])));
                }
            }
        }
    ...
    for (size_t i = 0; i < jsons.size(); ++i) {
        ...
        if (result.from_mitsuba) {
            result.scale = 0.66f;
            result.offset = {0.25f * result.scale,
                             0.25f * result.scale,
                             0.25f * result.scale};
        }
        ...
        if (json.contains("offset")) {
            result.offset = json["offset"].is_array()
                                ? Vector3f{float(json["offset"][0]),
                                           float(json["offset"][1]),
                                           float(json["offset"][2])}
                                : Vector3f{float(json["offset"]),
                                           float(json["offset"]),
                                           float(json["offset"])};
        }
        ...
        if (json.contains("aabb")) {
            ...
            float length = std::max(
                0.000001f,
                std::max(
                    std::max(std::abs(float(aabb[1][0]) - float(aabb[0][0])),
                             std::abs(float(aabb[1][1]) - float(aabb[0][1]))),
                    std::abs(float(aabb[1][2]) - float(aabb[0][2]))));
            result.scale = 1.f / length;
            result.offset = {((float(aabb[1][0]) + float(aabb[0][0])) * 0.5f) *
                                     -result.scale +
                                 0.5f,
                             ((float(aabb[1][1]) + float(aabb[0][1])) * 0.5f) *
                                     -result.scale +
                                 0.5f,
                             ((float(aabb[1][2]) + float(aabb[0][2])) * 0.5f) *
                                     -result.scale +
                                 0.5f};
        }
        ...
        if (json.contains("frames") && json["frames"].is_array()) {
            for (int j = 0; j < json["frames"].size(); ++j) {
                ...
                const Vector3f p = Vector3f{float(jsonmatrix_start[0][3]),
                                       ...
                                       result.scale +
                                   result.offset;
                const Vector3f q = Vector3f{float(jsonmatrix_end[0][3]),
                                            float(jsonmatrix_end[1][3]),
        ...
        if (json.contains("envmap") && result.envmap_resolution.isZero()) {
            ...
            if (equals_case_insensitive(envmap_path.extension(), "exr")) {
                result.envmap_data = load_exr(envmap_path.str(),
                                              result.envmap_resolution.x(),
                                              result.envmap_resolution.y());
                result.is_hdr = true;
            } else {
                result.envmap_data = load_stbi(envmap_path.str(),
                                               result.envmap_resolution.x(),
                                               result.envmap_resolution.y());
            }
        }
            ...
            pool.parallelForAsync<size_t>(
                ...
                [&, basepath, image_idx](size_t i) {
                    ...
                    if (equals_case_insensitive(path.extension(), "exr")) {
                        images[i_img] = load_exr_to_gpu(&res.x(),
                                                        &res.y(),
                                                        path.str().c_str(),
                                                        fix_premult);

                        if (image_type != ImageDataType::None &&
                            image_type != ImageDataType::Half) {
                            throw std::runtime_error{
                                "May not mix png and exr images."};
                        }
                        ...
                        uint8_t* img = stbi_load(
                            path.str().c_str(), &res.x(), &res.y(), &comp, 4);

                        fs::path alphapath =
                            basepath / (std::string{frame["file_path"]} +
                                        ".alpha."s + path.extension());
                        if (alphapath.exists()) {
                            int wa = 0, ha = 0;
                            uint8_t* alpha_img = stbi_load(
                                alphapath.str().c_str(), &wa, &ha, &comp, 4);
                            if (!alpha_img) {
                                throw std::runtime_error{
                                    "Could not load alpha image "s +
                                    alphapath.str()};
                            }
                            ScopeGuard mem_guard{
                                [&]() { stbi_image_free(alpha_img); }};
                            ...
                            for (int i = 0; i < res.prod(); ++i) {
                                img[i * 4 + 3] = uint8_t(
                                    ...
                                    srgb_to_linear(
                                        alpha_img[i * 4] *
                                        (1.f /
                                         255.f)));  // copy red channel of alpha
                        ...
                        fs::path maskpath =
                            path.parent_path() / (std::string{"dynamic_mask_"} +
                                                  path.basename() + ".png");
                        if (maskpath.exists()) {
                            int wa = 0, ha = 0;
                            ...
                            if (wa != res.x() || ha != res.y()) {
                                throw std::runtime_error{
                                    std::string{
                                        "Mask image has wrong resolution: "} +
                                    maskpath.str()};
                            }
                    ...
                    if (!result.image_resolution.isZero() &&
                        res != result.image_resolution) {
                        throw std::runtime_error{
                            "training images are not all the same size"};
                    }

                    fs::path rayspath =
                        path.parent_path() /
                        (std::string{"rays_"} + path.basename() + ".dat");
                    if (enable_ray_loading && rayspath.exists()) {
                        ...
                        std::ifstream rays_file{rayspath.str(),
                                                std::ios::binary};
                        rays_file.read((char*)rays[i_img],
                                       n_pixels * sizeof(Ray));
                    ...
                    // silly.
                    float x_fl =
                        read_focal_length(result.image_resolution.x(), "x");
                    float y_fl =
                        read_focal_length(result.image_resolution.y(), "y");

                    if (x_fl != 0) {
                        result.metadata[i_img].focal_length =
                            Vector2f::Constant(x_fl);
                        if (y_fl != 0) {
                            result.metadata[i_img].focal_length.y() = y_fl;
                        }
                    } else if (y_fl != 0) {
                        result.metadata[i_img].focal_length =
                            Vector2f::Constant(y_fl);
                    } else {
                    ...
                    for (int m = 0; m < 3; ++m) {
                        for (int n = 0; n < 4; ++n) {
                            ...
                            result.xforms[i_img].end(m, n) =
                                float(jsonmatrix_end[m][n]);
                        }
                    }
    ...
    tlog::success() << "Loaded " << images.size() << " images of size "
                    << result.image_resolution.x() << "x"
                    << result.image_resolution.y() << " after "
    ...
    if (image_type != ImageDataType::Half) {
        images_data_gpu_tmp.resize(img_size * images.size() *
                                   bytes_per_channel);
    }
    ...
    uint8_t* dst = image_type == ImageDataType::Half
                       ? (uint8_t*)result.images_data.data()
                       : (uint8_t*)images_data_gpu_tmp.data();
    pool.parallelFor<size_t>(0, result.n_images, [&](size_t i) {
        CUDA_CHECK_THROW(cudaMemcpy(dst + img_size * i * bytes_per_channel,
                                    images[i],
                                    img_size * bytes_per_channel,
                                    image_data_on_gpu
        ...
        if (has_rays) {
            ...
            CUDA_CHECK_THROW(cudaMemcpy(rays_dst + n_pixels * i,
                                        rays[i],
                                        n_pixels * sizeof(Ray),
                                        cudaMemcpyHostToDevice));
            free(rays[i]);
    ...
    if (image_type == ImageDataType::Byte) {
        linear_kernel(from_rgba32<__half>,
                      0,
                      nullptr,
                      n_pixels * result.n_images,
                      (uint8_t*)images_data_gpu_tmp.data(),
                      result.images_data.data(),
                      white_transparent,
                      black_transparent,
                      mask_color);
    } else if (image_type == ImageDataType::Float) {
        linear_kernel(from_fullp,
                      0,
                      nullptr,
                      img_size * result.n_images,
                      (float*)images_data_gpu_tmp.data(),
                      result.images_data.data());
    }
    if (sharpen_amount > 0.f) {
        ...
        float center_w =
            4.f +
            1.f / sharpen_amount;  // center_w ranges from 5 (strong sharpening)
                                   // to infinite (no sharpening)
        linear_kernel(sharpen,
                      0,
                      nullptr,
                      n_pixels * result.n_images,
                      result.image_resolution.x(),
                      result.images_data.data(),
                      images_data_2.data(),
                      center_w,
                      1.f / (center_w - 4.f));
    ...
    const dim3 blocks = {
        div_round_up((uint32_t)result.sharpness_resolution.x(), threads.x),
        div_round_up((uint32_t)result.sharpness_resolution.y(), threads.y),
        div_round_up((uint32_t)result.n_images, threads.z)};
    result.sharpness_data.enlarge(result.sharpness_resolution.x() *
                                  result.sharpness_resolution.y() *
                                  result.n_images);
    compute_sharpness<<<blocks, threads, 0, nullptr>>>(
        result.sharpness_resolution,
        result.image_resolution,
        result.n_images,
        result.images_data.data(),
        result.sharpness_data.data());
...
void
NerfDataset::set_training_image(int frame_idx, const float* pixels) {
