...
Testbed::NetworkDims
Testbed::network_dims_image() const {
    NetworkDims dims;
    dims.n_input = 2;
    dims.n_output = 3;
    dims.n_pos = 2;
...
__global__ void
sobol2_kernel(uint32_t n_elements,
              size_t base_idx,
              uint32_t seed,
              Vector2f* __restrict__ output) {
    uint32_t i = blockIdx.x * blockDim.x + threadIdx.x;
...
__global__ void
zip_kernel(uint32_t n_elements,
...
__global__ void
stratify2_kernel(uint32_t n_elements,
                 uint32_t log2_batch_size,
...
__global__ void
init_image_coords(Vector2f* __restrict__ positions,
                  Vector2i resolution,
...
__global__ void
shade_kernel_image(Vector2i resolution,
...
__global__ void
eval_image_kernel_and_snap(uint32_t n_elements,
...
Testbed::train_image(size_t target_batch_size,
    ...
    if (m_image.random_mode == ERandomMode::Halton) {
        ...
        if (m_image.random_mode == ERandomMode::Stratified) {
            ...
            if (!is_pot(batch_size, &log2_batch_size)) {
                ...
                linear_kernel(stratify2_kernel,
                              0,
                              stream,
                              n_elements,
                              log2_batch_size,
    ...
    if (m_image.type == EDataType::Float) {
        linear_kernel(eval_image_kernel_and_snap<float, 3>,
                      0,
                      stream,
                      n_elements,
        ...
        linear_kernel(eval_image_kernel_and_snap<__half, 3>,
                      0,
                      stream,
                      n_elements,
    ...
    for (size_t i = 0; i < n_steps; i += GRAPH_SIZE) {
        ...
        for (uint32_t j = 0; j < GRAPH_SIZE; ++j) {
            ...
            GPUMatrix<float> training_target_matrix(
                (float*)(m_image.training.targets.data() + training_offset),
                n_output_dims,
                batch_size);
    ...
    if (!m_render_ground_truth) {
        if (m_visualized_dimension >= 0) {
            GPUMatrix<float> positions_matrix(
                (float*)m_image.render_coords.data(), 2, n_elements);
            GPUMatrix<float> colors_matrix(
                (float*)m_image.render_out.data(), 3, n_elements);
...
Testbed::load_binary_image() {
    ...
    // 		cuFileHandleDeregister(cf_handle);
    // 		close(fd);
    // 		throw std::runtime_error{std::string{"buffer deregistration failed "} +
    // status.err};
    // 	}
...
__global__ void
image_coords_from_idx(const uint32_t n_elements,
                      uint32_t offset,
...
__global__ void
image_mse_kernel(const uint32_t n_elements,
...
Testbed::compute_image_mse() {
    ...
    for (uint32_t i = 0; i < n_batches; ++i) {
        ...
        GPUMatrix<float> predictions_matrix(
            (float*)(predictions.data()), n_output_dims, batch_size);

        linear_kernel(image_coords_from_idx,
                      0,
                      nullptr,
                      batch_size,
                      offset,
        ...
        linear_kernel(image_mse_kernel,
                      0,
                      nullptr,
                      batch_size,
                      targets.data(),
