...
template <typename T>
__device__ Eigen::Array4f
read_envmap(const T* __restrict__ envmap_data,
...
__device__ void
deposit_envmap_gradient(const tcnn::vector_t<T, 4>& value,
    ...
    auto deposit_val = [&](const tcnn::vector_t<T, 4>& value,
                           T weight,
                           Eigen::Vector2i pos) {
        if (pos.x() < 0) {
            pos.x() += envmap_resolution.x();
