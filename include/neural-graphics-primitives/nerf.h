...
struct RaysNerfSoa {
#ifdef __NVCC__
    void
    copy_from_other_async(const RaysNerfSoa& other, cudaStream_t stream) {
        CUDA_CHECK_THROW(cudaMemcpyAsync(rgba,
                                         other.rgba,
                                         size * sizeof(Eigen::Array4f),
                                         cudaMemcpyDeviceToDevice,
                                         stream));
        CUDA_CHECK_THROW(cudaMemcpyAsync(payload,
                                         other.payload,
    ...
    void
    set(Eigen::Array4f* rgba, NerfPayload* payload, size_t size) {
        this->rgba = rgba;
