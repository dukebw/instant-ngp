...
class SurfaceProvider {
   public:
    virtual cudaSurfaceObject_t
    surface() = 0;
    virtual cudaArray_t
    array() = 0;
    virtual Eigen::Vector2i
    resolution() const = 0;
    virtual void
    resize(const Eigen::Vector2i&) = 0;
};
...
class CudaSurface2D : public SurfaceProvider {
   public:
    ...
    void
    resize(const Eigen::Vector2i& size) override;
...
class GLTexture : public SurfaceProvider {
   public:
    ...
    void
    load(const float* data, Eigen::Vector2i new_size, int n_channels);
...
class CudaRenderBuffer {
   public:
    ...
    void
    resize(const Eigen::Vector2i& size);
    ...
    Eigen::Array4f*
    accumulate_buffer() const {
        return m_accumulate_buffer.data();
    }

    void
    clear_frame_buffer(cudaStream_t stream);

    void
    accumulate(cudaStream_t stream);
    ...
    void
    overlay_false_color(Eigen::Vector2i training_resolution,
                        bool to_srgb,
                        int fov_axis,
                        cudaStream_t stream,
    ...
    void
    set_color_space(EColorSpace color_space) {
    ...
    void
    set_tonemap_curve(ETonemapCurve tonemap_curve) {
