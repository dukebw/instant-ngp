...
struct Triangle {
    ...
    sample_uniform_position(const Eigen::Vector2f& sample) const {
    ...
    NGP_HOST_DEVICE float
    ray_intersect(const Eigen::Vector3f& ro, const Eigen::Vector3f& rd) const {
        Eigen::Vector3f n;
    ...
    NGP_HOST_DEVICE float
    distance(const Eigen::Vector3f& pos) const {
        return std::sqrt(distance_sq(pos));
    }

    NGP_HOST_DEVICE bool
    point_in_triangle(const Eigen::Vector3f& p) const {
    ...
    NGP_HOST_DEVICE Eigen::Vector3f
    closest_point_to_line(const Eigen::Vector3f& a,
    ...
    NGP_HOST_DEVICE Eigen::Vector3f
    closest_point(Eigen::Vector3f point) const {
    ...
    NGP_HOST_DEVICE float
    centroid(int axis) const {
        return (a[axis] + b[axis] + c[axis]) / 3;
    }

    NGP_HOST_DEVICE void
    get_vertices(Eigen::Vector3f v[3]) const {
        v[0] = a;
        v[1] = b;
        v[2] = c;
    }
