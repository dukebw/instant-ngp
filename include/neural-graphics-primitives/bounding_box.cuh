...
NGP_HOST_DEVICE inline void
project(Eigen::Vector3f points[N_POINTS],
...
struct BoundingBox {
    ...
    enlarge(const BoundingBox& other) {
        min = min.cwiseMin(other.min);
        max = max.cwiseMax(other.max);
    }

    NGP_HOST_DEVICE void
    enlarge(const Triangle& tri) {
        enlarge(tri.a);
        enlarge(tri.b);
        enlarge(tri.c);
    }
    ...
    NGP_HOST_DEVICE void
    enlarge(const Eigen::Vector3f& point) {
        min = min.cwiseMin(point);
        max = max.cwiseMax(point);
    }

    NGP_HOST_DEVICE void
    inflate(float amount) {
        min -= Eigen::Vector3f::Constant(amount);
        max += Eigen::Vector3f::Constant(amount);
    }

    NGP_HOST_DEVICE Eigen::Vector3f
    diag() const {
        return max - min;
    }

    NGP_HOST_DEVICE Eigen::Vector3f
    relative_pos(const Eigen::Vector3f& pos) const {
    ...
    intersection(const BoundingBox& other) const {
    ...
    NGP_HOST_DEVICE bool
    intersects(const BoundingBox& other) const {
    ...
    NGP_HOST_DEVICE bool
    intersects(const Triangle& triangle) const {
    ...
    NGP_HOST_DEVICE bool
    contains(const Eigen::Vector3f& p) const {
    ...
    distance(const Eigen::Vector3f& p) const {
        return sqrt(distance_sq(p));
    }

    NGP_HOST_DEVICE float
    distance_sq(const Eigen::Vector3f& p) const {
        return (min - p).cwiseMax(p - max).cwiseMax(0.0f).squaredNorm();
    }

    NGP_HOST_DEVICE float
    signed_distance(const Eigen::Vector3f& p) const {
    ...
    NGP_HOST_DEVICE void
    get_vertices(Eigen::Vector3f v[8]) const {
