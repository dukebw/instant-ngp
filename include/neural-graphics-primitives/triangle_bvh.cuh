...
__host__ __device__ std::pair<int, float>
trianglebvh_ray_intersect(const Eigen::Vector3f& ro,
...
class TriangleBvh {
   public:
    virtual void
    signed_distance_gpu(uint32_t n_elements,
                        EMeshSdfMode mode,
    ...
    virtual void
    ray_trace_gpu(uint32_t n_elements,
                  Eigen::Vector3f* gpu_positions,
                  Eigen::Vector3f* gpu_directions,
                  const Triangle* gpu_triangles,
                  cudaStream_t stream) = 0;
    virtual bool
    touches_triangle(const BoundingBox& bb,
                     const Triangle* __restrict__ triangles) const = 0;
    virtual void
