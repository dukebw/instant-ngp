...
struct NerfDataset {
    ...
    void
    set_training_image(int frame_idx, const float* pixels);

    Eigen::Vector3f
    nerf_direction_to_ngp(const Eigen::Vector3f& nerf_dir) {
    ...
    nerf_matrix_to_ngp(const Eigen::Matrix<float, 3, 4>& nerf_matrix) {
    ...
    Eigen::Matrix<float, 3, 4>
    ngp_matrix_to_nerf(const Eigen::Matrix<float, 3, 4>& ngp_matrix) {
    ...
    void
    nerf_ray_to_ngp(Ray& ray, bool scale_direction = false) {
