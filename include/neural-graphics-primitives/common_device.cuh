...
inline __host__ __device__ float
srgb_to_linear_derivative(float srgb) {
    if (srgb <= 0.04045f) {
        return 1.0f / 12.92f;
    } else {
...
inline __host__ __device__ Eigen::Array3f
srgb_to_linear_derivative(const Eigen::Array3f& x) {
    return {srgb_to_linear_derivative(x.x()),
            srgb_to_linear_derivative(x.y()),
...
inline __host__ __device__ float
linear_to_srgb_derivative(float linear) {
    if (linear < 0.0031308f) {
        return 12.92f;
    } else {
...
inline __host__ __device__ Eigen::Array3f
linear_to_srgb_derivative(const Eigen::Array3f& x) {
    return {linear_to_srgb_derivative(x.x()),
            linear_to_srgb_derivative(x.y()),
            (linear_to_srgb_derivative(x.z()))};
}

template <uint32_t N_DIMS, typename T>
__host__ __device__ Eigen::Matrix<float, N_DIMS, 1>
...
template <uint32_t N_DIMS, typename T>
__device__ void
deposit_image_gradient(const Eigen::Matrix<float, N_DIMS, 1>& value,
    ...
    auto deposit_val = [&](const Eigen::Matrix<float, N_DIMS, 1>& value,
                           T weight,
                           Eigen::Vector2i pos) {
        pos.x() = std::max(std::min(pos.x(), resolution.x() - 1), 0);
        pos.y() = std::max(std::min(pos.y(), resolution.y() - 1), 0);
...
camera_distortion(const T* extra_params, const T u, const T v, T* du, T* dv) {
...
template <typename T>
__device__ __host__ inline void
iterative_camera_undistortion(const T* params, T* u, T* v) {
...
pixel_to_ray_pinhole(uint32_t spp,
...
f_theta_undistortion(const Eigen::Vector2f& uv,
                     const Eigen::Vector2f& screen_center,
    ...
    float alpha = camera_distortion.params[0] +
                  norm * (camera_distortion.params[1] +
                          norm * (camera_distortion.params[2] +
                                  norm * (camera_distortion.params[3] +
                                          norm * camera_distortion.params[4])));
...
inline __host__ __device__ float
fov_to_focal_length(int resolution, float degrees) {
...
template <typename T>
__global__ void
from_rgba32(const uint64_t num_pixels,
...
log_space_lerp(const Eigen::Matrix<float, 3, 4>& begin,
               const Eigen::Matrix<float, 3, 4>& end,
               float t);

tcnn::GPUMemory<float>
load_exr(const std::string& filename, int& width, int& height);
tcnn::GPUMemory<float>
load_stbi(const std::string& filename, int& width, int& height);
