...
struct less<ngp::Vector4i16> {
    bool
    operator()(const ngp::Vector4i16& a, const ngp::Vector4i16& b) const {
...
struct hash<ngp::Vector4i16> {
    size_t
    operator()(const ngp::Vector4i16& x) const {
...
class TriangleOctree {
   public:
    ...
    build(const TriangleBvh& bvh,
        ...
        for (auto& node : m_nodes) {
            for (uint32_t i = 0; i < 8; ++i) {
                ...
                generate_dual_coords(
                    m_dual_nodes[child_idx], node.depth + 1, child_pos);
            }
        }
    ...
    traverse(const TriangleOctreeNode* nodes,
...
#pragma unroll
    ...
    ray_intersect(const TriangleOctreeNode* nodes,
                  int max_depth,
    ...
    const TriangleOctreeNode*
    nodes_gpu() const {
        return m_nodes_gpu.data();
    }

    const TriangleOctreeDualNode*
    dual_nodes_gpu() const {
