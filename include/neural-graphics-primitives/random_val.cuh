...
inline __host__ __device__ Eigen::Vector2f
dir_to_cylindrical(const Eigen::Vector3f& d) {
...
inline __host__ __device__ Eigen::Vector2f
dir_to_spherical_unorm(const Eigen::Vector3f& d) {
    const float cos_theta = fminf(fmaxf(d.z(), -1.0f), 1.0f);
...
inline __host__ __device__ Eigen::Vector2f
square2disk_shirley(const Eigen::Vector2f& square) {
    float phi, r;
    float a = square.x();
    float b = square.y();
...
inline __host__ __device__ __device__ Eigen::Vector3f
cosine_hemisphere(const Eigen::Vector2f& u) {
...
template <typename RNG>
inline __host__ __device__ Eigen::Vector3f
random_dir_cosine(RNG& rng) {
    return cosine_hemisphere(random_val_2d(rng));
}

template <typename RNG>
inline __host__ __device__ Eigen::Vector3f
random_val_3d(RNG& rng) {
...
inline __host__ __device__ uint32_t
sobol(uint32_t index, uint32_t dim) {
...
inline __host__ __device__ uint32_t
laine_karras_permutation(uint32_t x, uint32_t seed) {
    x += seed;
    x ^= x * 0x6c50b47cu;
    x ^= x * 0xb82f1e52u;
    x ^= x * 0xc7afe638u;
...
inline __host__ __device__ Eigen::Vector2f
ld_random_val_2d(uint32_t index, uint32_t seed) {
    constexpr float S = float(1.0 / (1ull << 32));
...
inline __host__ __device__ float
ld_random_val(uint32_t index, uint32_t seed, uint32_t dim = 0) {
    constexpr float S = float(1.0 / (1ull << 32));
    index = nested_uniform_scramble_base2(index, seed);
